...
- Срок действия: `12/30`, CVC: `123`, имя: `Demo Buyer`.
- На странице checkout есть блок "Оплата для презентации": кнопка "Подставить" заполняет карту автоматически.

Полный номер карты, CVC и срок действия не сохраняются. В базе остаются только
статус, reference и последние четыре цифры.

## Быстрый сценарий для презентации

1. Запустить backend и frontend, открыть сайт: `http://localhost:5173`.
2. Войти покупателем: `customer@woodstyle.com` / `Customer123!`.
3. Добавить товар в корзину и перейти в checkout.
4. В блоке оплаты нажать "Подставить" на успешной демо-карте.
5. Нажать кнопку оплаты. Заказ должен перейти в статус `paid`.

Для демонстрации ошибки оплаты используйте карту `4000 0000 0000 0002`.
